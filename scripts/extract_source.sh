...
fi
{
    ...
    write_log_msg "Extracting archives…"
    for f in *.tar.*; do
        case "$f" in
        *.sig) continue ;;
        esac
    ...
    if [[ ${USE_GCC_SYMLINKS:-true} == true ]]; then
        {
            write_log_msg "Creating GCC dependency symbolic links..."
            for dep in gmp isl mpc mpfr; do
                ...
                if [ -z "$pkgdir" ]; then
                    write_log_msg "Skipping $dep (not found)" --level="$WARN" --std
                    continue
                fi

                write_log_msg "Linking $PWD/$dep -> $pkgdir"
                ln -sfn "$pkgdir" "$dep"
            done
        }
    fi
